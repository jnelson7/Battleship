...
void OS_Wait(Sema4Type *semaPt){
	...
	(semaPt->Value)--; // semaphore is decremented
	OS_EnableInterrupts();	
}
...
void OS_Signal(Sema4Type *semaPt){
	// Your code here.
	OS_DisableInterrupts();
	(semaPt->Value)++; // semaphor is incremented 
	OS_EnableInterrupts();
}
...
void OS_InitSemaphore(Sema4Type *semaPt, long value){
	OS_DisableInterrupts();
	semaPt->Value = value; // the value is initialized before it is ran
	OS_EnableInterrupts();
}
...
void OS_bSignal(Sema4Type *semaPt){
	// Your code here.
	OS_DisableInterrupts();
	semaPt->Value = 1; // for a binary semaphore signal, the value is set to 1;
	OS_EnableInterrupts();
}
...
void OS_Sleep(unsigned long sleepTime){
	// Your code here.
	tcbs[RunPt->id].sleepCt = sleepTime; // first step for sleeping
	OS_Suspend(); // 2nd step for sleeping
}
...
void Scheduler(void){
	RunPt = RunPt->next; 
	// if thread is sleeping the next thread is ran until I find one not sleeping
	while(RunPt->sleepCt){ 
		RunPt = RunPt->next; 
	}
}
...
void Timer2A_Handler(void){ 
	...
	for(i = 0; i < NUMTHREADS; i++){ // goes through all threads
		if(tcbs[i].sleepCt>0 && tcbs[i].available==0){ // only does computation if the thread is sleeping
			tcbs[i].sleepCt--; // Sleep is decremented from the threads that are sleeping
		}
	}
}
